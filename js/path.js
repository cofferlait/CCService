		...
		function selectPoints() {
            ...
            var  pointStyle = {
                ...
                startMeasure:0,
                endMeasure:1000,
                externalGraphic:"fx.png",
                graphicWidth:40,
                graphicHeight:40,
                rotation:-30
            };
            var urlFangXiang="http://localhost:8090/iserver/services/spatialanalyst-changchun/restjsr/spatialanalyst";
			function alertFangXiang()
			{
				alert(routeObj);
			}
			function addFangXiang(fxGeometry)
			{				
            	var m;
            	for(m=40;m<fxGeometry.length;m=m+100)
            	{
            		RouteLocator(m);
            	}
			}
			//"路由定点"操作，其中将routeObj和里程值作为参数传递到参数对象中
            function RouteLocator(m) {
                if(feature != null)
                {

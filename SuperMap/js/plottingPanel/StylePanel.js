...
SuperMap.Plotting.StylePanel = new SuperMap.Class({
    ...
    addEditLayer: function(editLayer){
        ...
        editLayer.events.register("beforefeaturemodified", this, this.showFeatureProperty);
    ...
    removeEditLayer : function(editLayer)
    {
        for(var i = 0; i < this.editLayers.length; i++)
        {
            if(editLayer === this.editLayers[i])
            {
                this.editLayers[i].events.unregister("beforefeaturemodified", this, this.showFeatureProperty);
    ...
    collectionPropertyGridRows: function(selectfeature)
    {
        ...
        var dotSymbolScaleObj  = new Object();
        dotSymbolScaleObj.name = this.displayName[1];
        dotSymbolScaleObj.value = selectfeature.geometry.getScale();
        dotSymbolScaleObj.group = this.group[0];
        dotSymbolScaleObj.editor = "text";
        ...
        if(selectfeature.geometry.symbolType === SuperMap.Plot.SymbolType.DOTSYMBOL){
            ...
            if(selectfeature.geometry.symbolType === SuperMap.Plot.SymbolType.TEXTSYMBOL){
                ...
                for (var i = 0; i < selectfeature.geometry.getSubSymbols().length; i++) {
                    ...
                    objectSubCode.value = selectfeature.geometry.getSubSymbols()[i];
        ...
        if (updated != null) {
            switch(updated.name)
            {
                ...
                case this.displayName[1]:
                    selectfeature.geometry.setScale(parseFloat(updated.value));
                    break;
                ...
                case this.displayName[14]:
                    selectfeature.style.fontSize = parseFloat(updated.value).toString() + "em";
                    break;
    ...
    getAnnotationRows: function(geometry){
        ...
        if(geometry.symbolData.middleMarkExist)
    ...
    {
        var symbolRanks = geometry.symbolData.symbolRanks;
